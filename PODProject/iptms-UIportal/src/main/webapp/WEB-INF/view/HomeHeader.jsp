<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"
    isELIgnored="false"
    %>

<html>
<head>
<%@ include file="headlink.jsp"%>
<title></title>
<link rel="stylesheet" href="style/style.css">

</head>
<body>
 <header>
        <div class="jumbotron-fluid bg-danger text-white">
            <nav class="navbar navbar-expand-md navbar-dark bg-primary text-white justify-content-center" id="navbar" >
                <a href="#" class="navbar-brand"><i class="material-icons">bloodtype</i>International Patient Treatment Management Service</a>
                <button class="navbar-toggler" data-toggle="collapse" data-target="#truyumbar">
                    <span class="navbar-toggler-icon"></span>
                </button>
                <div class="container">
                    <div class="collapse navbar-collapse" id="truyumbar">
                        <ul class="navbar-nav ml-auto">
                            <li class="nav-item"><a href="/main-portal" class="nav-link btn-danger">Main Portal</a></li>
                        </ul>
                    </div>
                </div>
            </nav>
         </div>


    </header>

</body>
</html>